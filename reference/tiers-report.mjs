...
import { lcg, UNITS } from "./lib/rand.mjs";
...
// The stage-1 token count must come from the ENGINE, never be re-derived here.
// This line used to be `Math.round(e.stage1Chars / 4)` with the comment "same
// chars/4 rule as rawTextTokens". 0.19 replaced that divisor with a measured
// class-weighted estimator, so the comment silently became false and this table
// started reporting L1 whitespace - which is LOSSLESS - as a 49% saving,
// because `raw` came from the engine while the tiers came from the stale rule.
// A second implementation of a number is a second answer waiting to disagree.
const textTok = (e) => e.stage1Tokens;
...
for (const c of corpora) {
  ...
  // text tiers (never rendered; token counts come from the engine)
  const text = {
...
md.push("");
md.push("Token counts come from the engine's measured class-weighted estimator (EVALS \u00a79), not a chars/4 guess - so a lossless stage reads 0%. L1 and table are lossless; L2-L4 reword prose; distill drops repeats but keeps every error line.");
md.push("");
