...
- **The exact bytes must survive.** Secrets and credentials are **auto-refused — never imaged**. Dense random strings can misread silently, so the `verbatim` sidecar ships uuids/hashes/ids as text; edit-targets should stay text.
