//! SHA-256 (FIPS 180-4), std only — stash ids need a stable content hash
//! and a crypto crate would be the only consumer-facing dependency for it.
...
pub fn digest(data: &[u8]) -> [u8; 32] {
    ...
    for i in 0..blocks {
        compress(&mut h, tail[i * 64..i * 64 + 64].try_into().unwrap());
    }
    let mut out = [0u8; 32];
...
pub fn hex(data: &[u8]) -> String {
    use std::fmt::Write;
    digest(data).iter().fold(String::with_capacity(64), |mut s, b| {
        let _ = write!(s, "{b:02x}");
        s
    })
}
...
mod tests {
    ...
    fn fips_180_4_vectors() {
        ...
        // two-block message: exercises the rem+9 > 64 padding path
        assert_eq!(
